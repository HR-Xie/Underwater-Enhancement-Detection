...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None):
        ...
        self.transform = A.Compose([
            A.PadIfNeeded(min_height=self.ps_h, min_width=self.ps_w, border_mode=cv2.BORDER_REFLECT),
            # 这一步会在原图上随机“挖”一个 h * w 的高清局部块
            A.RandomCrop(height=self.ps_h, width=self.ps_w),
            A.Flip(p=0.3),
